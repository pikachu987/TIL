...
public class MemberRegistValidator implements Validator{
	...
	public boolean supports(Class<?> clazz) {
		return MemberRegistValidator.class.isAssignableFrom(clazz);
	}
	...
	public void validate(Object target, Errors errors) {
		...
		if(regReq.hasPassword()){
			if(regReq.getPassword().length() < 5) errors.rejectValue("password", "shortPassword");
			else if(!regReq.isSamePasswordConfirmPassword()) errors.rejectValue("confirmPassword", "notSame");
		}
